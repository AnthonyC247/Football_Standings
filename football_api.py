...
def fetch_league_standings(league=None, season=None):
    ...
    if data and 'response' in data:
        ...
        df = df[['rank', 'team.name', 'points', 'goalsDiff', 'group', 'all.played', 'all.win', 'all.draw', 'all.lose']]
        df.columns = ['Rank', 'Team', 'Points', 'Goals Difference', 'Group', 'Played', 'Wins', 'Draws', 'Losses']
