from football_api import fetch_leagues, fetch_league_standings, \
                         fetch_team_standings, fetch_team_info, \
                         fetch_team_stats
from display import display_league_standings, display_team_info, \
                    display_team_stats, display_team_standings, \
                    display_leagues
...
def main():
    while True:
        ...
        print("6. Exit")
        choice = input("What would you like to fetch? Enter \
                        your choice (1-6): ")
        ...
        if choice == '1':
            ...
            season = input("Enter the season year \
                            (e.g., 2021): ")
            success = fetch_team_stats(name=team_name,
                                       league=league_name,
                                       season=season)
            if not success:
                continue
            ...
            if not success:
                continue
            # Display league standings after fetching
        ...
        continue_choice = input("Do you want to keep fetching data? \
                                (yes/no): ").strip().lower()
